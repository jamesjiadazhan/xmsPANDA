eig_norm1 <-
    function(m, treatment, prot.info, write_to_file = "", plot.trend = FALSE) {
        ...
        rownames(pres) = prot.info[
            !(prot.info[, 1] %in% rownames(pmiss)),
            1
        ]
        ...
        if (write_to_file != "") {
            utils::write.table(complete,
                file = write_to_file, append = FALSE,
                quote = FALSE, sep = "\t", eol = "\n", na = "NaN",
                dec = ".", row.names = TRUE, col.names = TRUE, qmethod = c(
                    "escape",
                    "double"
                )
            )
        ...
        if (n.u.treatment > 1) {
            ...
            attach(TREATS)
            mod.c = stats::model.matrix(lm.fm$lm.formula,
                data = TREATS,
