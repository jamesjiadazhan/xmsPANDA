    ...
    function(xmis, maxiter = 10, ntree = 100, variablewise = FALSE,
        ...
        if (parallelize %in% c("variables", "forests")) {
            ...
            if (getDoParWorkers() > p) {
                stop(
                    "The number of parallel cores should not exceed the number of variables (p=",
                    p, ")"
                )
            }
        }
        ...
        if (iter == maxiter) {
            if (any(is.na(xtrue))) {
                ...
                out <- list(
                    ximp = Ximp[[iter]], OOBerror = OOBerr,
                    error = err
                )
            }
        } else {
            if (any(is.na(xtrue))) {
                ...
                out <- list(
                    ximp = Ximp[[iter - 1]], OOBerror = OOBerrOld,
