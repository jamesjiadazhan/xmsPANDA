    ...
    function(feature_table_file, class_labels_file, X = NA, Y = NA, parentoutput_dir, newdevice = FALSE,
        ...
        lapply(2:dim(mtcars)[2], function(i) {
            ...
            if (plottype == "barplot") {
                ...
                if (FALSE) {
                    barplot1 <- ggplot(data = myData) +
                        geom_bar(aes(x = Samples, y = Intensity, fill = Class), stat = "identity") +
                        facet_wrap(~Var2) +
                        scale_fill_manual(values = unique(barplot.col.opt1)) +
                        ggtitle(mzlabel_cur) +
                        ylab(ylabel) +
                        xlab("Samples") +
                        theme_bw() +
                ...
                if (plottype == "point") {
                    ...
                    if (plottype == "ggplot") {
                        p <- ggplot(data = myData, aes(x = Class, y = Intensity, fill = Class)) +
                            scale_fill_manual(values = c(barplot.col.opt1)) +
                            ylab(ylabel)
