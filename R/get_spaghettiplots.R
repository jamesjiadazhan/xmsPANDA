get_spaghettiplots <-
    function(X, Y, feature_table_file, parentoutput_dir, class_labels_file, boxplot.col.opt = "journal", alphacol = 0.3, newdevice = TRUE, cex.plots = 0.8,
             ...
             boxplot.type = "ggplot", study.design = c(
                ...
                if (length(class_labels_levels) >= 2) {
                    if (length(class_labels_levels) >= 1) {
                        ...
                        if (multiple.groups == FALSE) {
                            p <- ggplot(temp_dm2, aes(y = as.numeric(Feature), x = Class, group = SubjectID)) +
                                geom_line() +
                                labs(title = mzname)
                        } else {
                            ...
                            if (ggplot.type1 == TRUE) {
                                p <- ggplot(temp_dm2, aes(y = as.numeric(Feature), x = Factor1, group = SubjectID, color = SubjectID)) +
                                    geom_point() +
                                    geom_line(size = 1.25) +
                                    labs(title = mzname) +
                                    facet_wrap(~Factor2, scale = "free_x")
                            } else {
                                p <- ggplot(temp_dm2, aes(y = as.numeric(Feature), x = Factor2, group = SubjectID, color = SubjectID)) +
                                    geom_point() +
                                    geom_line(size = 1.25) +
                                    labs(title = mzname) +
                                    facet_wrap(~Factor1, scale = "free_x")
                            }
                        }
                        ...
                        if (FALSE) {
                            ...
                            ) + scale_fill_manual(values = (col_vec[1:length(unique(temp_dm2$ID))])) + scale_y_continuous(breaks = scales::pretty_breaks(n = 10)) + theme(axis.text.x = element_text(angle = 45, hjust = 1))
                        }
