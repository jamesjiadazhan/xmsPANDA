    ...
    function(multiME, setLabels, letterSubPlots = FALSE, Letters = NULL,
        ...
        if (plotHeatmaps)
            for (i.row in (1:nSets)) for (i.col in (i.row:nSets)) {
                ...
                if (letterSubPlots) {
                    letter = paste(substring(Letters,
                        first = letter.ind,
                        last = letter.ind
                    ), ".  ", sep = "")
                } else {
                ...
                if (i.row == i.col) {
                    ...
                    corrplot::corrplot(cor.dif,
                        col = heatmap_cols, cl.cex = 0.9, tl.cex = 0.45, title = paste("Delta correlation: ", setLabels[i.row], " vs ", setLabels[i.col], sep = ""),
                    ...
                    labeledBarplot(means, names(multiME[[i.col]]$data),
                        main = paste(method, " score for ", i.row, " vs ", i.col, ":", signif(
                            sum_dp,
                            2
                        )), ylim = c(0, 1),
