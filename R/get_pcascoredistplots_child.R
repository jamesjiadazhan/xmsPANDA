get_pcascoredistplots_child <-
    function(X, Y, feature_table_file, parentoutput_dir, class_labels_file,
        ...
        if (dim(data_m)[1] > 2) {
            res <- get_pca(
                X = data_m, samplelabels = classgroup, legendlocation = legendlocation, filename = filename, ncomp = 10, pcacenter = pcacenter, pcascale = pcascale, legendcex = 0.5, outloc = getwd(),
            ...
            get_pooled_sp <- function(n1, n2, S1, S2) {
                a <- (n1 - 1) * S1;b <- (n2 - 1) * S2;c <- (n1 + n2 - 2);return((a + b) / c)
            }
            ...
            if (analysistype == "regression") {
                ...
                plot_res <- suppressMessages(ggscatter(df.sub1,
                    x = "Outcome", y = "PCscore", xlab = "Outcome", ylab = "PC1score",
                    ...
                    conf.int = TRUE
                )) + theme(plot.title = element_text(hjust = 0.5, size = 10)) + stat_cor(
                    method = "spearman",
                ...
                if (pcnum_limit > 1) {
                    ...
                    plot_res <- suppressMessages(ggscatter(df.sub1,
                        x = "Outcome", y = "PCscore", xlab = "Outcome", ylab = "PC2score",
                ...
                if (pcnum_limit > 2) {
                    ...
                    plot_res <- suppressMessages(ggscatter(df.sub1,
                        x = "Outcome", y = "PCscore", xlab = "Outcome", ylab = "PC3score",
                    ...
                    if (dim(classlabels_orig)[2] > 2) {
                        ...
                        lapply(1:pcnum_limit, function(pc) {
                            ...
                            if (pairedanalysis == TRUE) {
                                ...
                                plot_res <- suppressWarnings(ggplot(df.summary, aes(
                                    x = as.factor(x), y = PCscore,
                                ...
                                if (timeseries.lineplots == TRUE) {
                                    ...
                                    if (is.na(lineplot.lty.option) == TRUE) {
                                        plot_res <- suppressWarnings(ggplot(df.summary, aes(x = as.factor(x), y = PCscore, colour = as.factor(Class), linetype = Class)) +
                                            geom_point(size = pca.cex.val, shape = shape_vec1) +
                                        ...
                                        plot_res <- suppressWarnings(ggplot(df.summary, aes(x = as.factor(x), y = PCscore, colour = as.factor(Class))) +
                                            geom_point(size = pca.cex.val, shape = shape_vec1) +
                                            geom_line(aes(group = Class), size = sizeval) +
                                            labs(colour = "Class") +
                                            geom_errorbar(aes(ymin = ymin, ymax = ymax), size = 1, width = 0.1) +
                                    ...
                                    plot_res <- suppressWarnings(ggplot(df.summary, aes(x = as.factor(x), y = PCscore, color = as.factor(Class))) +
                                        geom_point(size = pca.cex.val, shape = shape_vec1) +
                                        labs(color = "Class") +
                                        geom_errorbar(aes(ymin = ymin, ymax = ymax), size = 1, width = 0.1) +
                                        xlab("") +
                                        scale_color_manual(values = unique(class_col_vec)) +
                                        theme_bw() +
                                        theme(
                                            panel.border = element_blank(), panel.grid.major = element_blank(),
                                            panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = sizeval),
                                        ...
                                        scale_y_continuous(breaks = scales::pretty_breaks(n = 10)) +
                                        scale_fill_manual(values = unique(class_col_vec)))
...

                            {
                                if (pairedanalysis == FALSE) {
                                    if (timeseries.lineplots == TRUE) {
                                        ...
                                        +
                                            geom_point(size = cex.plots) +
                                            geom_line(size = sizeval)
                                            +
                                            geom_errorbar(aes(ymin = ymin, ymax = ymax), size = 1, width = 0.1) +
                                            xlab("")
                                            +
                                        ...
                                        if (length(unique(class_col_vec)) == 1) {
                                            if (length(unique(class_col_vec)) == 1 && analysistype == "regression") {
                                                ...
                                                plot_res <- suppressMessages(ggscatter(df.sub1,
                                                    x = "x", y = "PCscore", xlab = "Outcome", col = class_col_vec,
                                                    palette = "jco", shape = 20, size = 3, # Points color, shape and size
                                                    add = "reg.line", # Add regressin line
                                                    add.params = list(color = "#0072B2", fill = "lightgray"), # Customize reg. line
                                                    conf.int = TRUE
                                    ...
                                    sizeval = 1.5
                                    plot_res <- suppressWarnings(ggplot(df.summary, aes(x = as.factor(x), y = PCscore, group = 1)) +
                                        geom_point(size = pca.cex.val) +
                                        geom_line(size = sizeval) +
                                        geom_errorbar(aes(ymin = ymin, ymax = ymax), size = 1, width = 0.1) +
                                        scale_color_manual(values = unique(class_col_vec)) +
