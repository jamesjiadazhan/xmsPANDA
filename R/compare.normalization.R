compare.normalization <-
    function(Xmat = NA, Ymat = NA, Zmat = NA, feature_table_file = NA, parentoutput_dir, class_labels_file = NA, num_replicates = 3, feat.filt.thresh = NA, summarize.replicates = TRUE, summary.method = "mean",
             ...
             normalization.method = c(
                 "all", "log2quantilenorm", "log2transform", "znormtransform", "lowess_norm", "quantile_norm", "rangescaling",
                 "paretoscaling", "mstus", "eigenms_norm", "vsn_norm", "sva_norm", "tic_norm", "cubicspline_norm", "mad_norm"
        ...
        valid_norm_options <- c(
            "log2quantilenorm", "log2transform", "znormtransform", "lowess_norm", "quantile_norm", "rangescaling", "paretoscaling", "mstus",
        ...
        data_matrix_list <- lapply(1:length(normalization.method), function(i) {
            if (normalization.method[i] %in% valid_norm_options) {
                ...
                data_matrix_list <- data_preprocess(
                    Xmat = Xmat, Ymat = Ymat, feature_table_file = feature_table_file, parentoutput_dir = outloc, class_labels_file = class_labels_file, num_replicates = num_replicates, feat.filt.thresh = NA, summarize.replicates = summarize.replicates,
                    ...
                    featselmethod = featselmethod, pairedanalysis = pairedanalysis, normalization.method = normalization.method[i], input.intensity.scale = input.intensity.scale, log2.transform.constant = log2.transform.constant
                ) # ,silent=TRUE)
                ...
                if (is(data_matrix_list, "try-error")) {
                    ...
                    pcares <- get_pcascoredistplots(
                        X = X, Y = Y, feature_table_file = NA, parentoutput_dir = outloc, class_labels_file = NA, sample.col.opt = "journal",
                        ...
                        lineplot.col.opt = "black", lineplot.lty.option = c("solid", "dashed", "dotted", "dotdash", "longdash", "twodash"), timeseries.lineplots = FALSE, pcacenter = TRUE, pcascale = TRUE, alphabetical.order = alphabetical.order, study.design = analysistype, lme.modeltype = lme.modeltype
                    ) # ,silent=TRUE)
                    ...
                    mycl_metabs <- cutreeDynamic(f1,
                        distM = as.matrix((1 - WGCNA::cor(x))), method = "hybrid", cutHeight = 0.95,
                    ...
                    plotDendroAndColors(f1, mycl_samples,
                        rowText = mycl_samples, groupLabels = c("Cluster"),
                    ...
                    if (FALSE) {
                        hca_res <- get_hca(
                            parentoutput_dir = outloc, X = X, Y = Y, heatmap.col.opt = heatmap.col.opt, cor.method = cor.method, is.data.znorm = FALSE, analysismode = "classification",
                    ...
                    return(list(
                        "data_matrix_list" = data_matrix_list, "Silhouette.sample" = Silhouette.samples,
        ...
        if (FALSE) {
            plot(unlist(Silhouette.feature),
                type = "o", ylim = c(0, 1), col = "#0072B2", lwd = 2, xaxt = "n", xlab = "", ylab = "Value",
            ...
            legend("topright",
                legend = c("Silhouette.features", "Silhouette.samples", "Gap.feautres", "Gap.samples", "Adjusted Rand Index"),
                lty = c(1, 2, 5, 3, 4), col = c(
                    "#0072B2", "#E69F00",
                    "#009E73", "#56B4E9",
                    "#D55E00"
                )
        ...
        return(list(
            data_matrix_list = data_matrix_list, normalization.methods = normalization.method,
