diffexp.child <-
    function(Xmat, Ymat, feature_table_file, parentoutput_dir, class_labels_file, num_replicates, feat.filt.thresh, summarize.replicates, summary.method,
        ...
        if (analysismode == "regression") {
            ...
            if (is.na(all(diff(match(rnames_xmat, rnames_ymat)))) == FALSE) {
                if (all(diff(match(rnames_xmat, rnames_ymat)) > 0) == TRUE) {
                    ...
                    data_matrix <- data_preprocess(
                        Xmat = Xmat, Ymat = Ymat, feature_table_file = feature_table_file, parentoutput_dir = parentoutput_dir, class_labels_file = NA, num_replicates = num_replicates, feat.filt.thresh = NA, summarize.replicates = summarize.replicates, summary.method = summary.method,
        ...
        if (sample.col.opt == "default") {
            col_vec <- c(
                "#CC0000", "#AAC000", "blue", "mediumpurple4", "mediumpurple1", "blueviolet", "cornflowerblue", "cyan4", "skyblue",
                        ...
                        if (sample.col.opt == "terrain") {
                            ...
                            if (sample.col.opt == "colorblind") {
                                ...
                                if (length(class_labels_levels) < 9) {
                                    ...
                                    col_vec <- c(
                                        "#0072B2", "#E69F00", "#009E73", "#56B4E9", "#D55E00", "#CC79A7", "#E64B35B2", "#4DBBD5B2", "#00A087B2", "#3C5488B2", "#F39B7FB2", "#8491B4B2", "#91D1C2B2", "#DC0000B2", "#7E6148B2",
                                ...
                                if (length(check_brewer) > 0) {
                                    ...
                                    if (sample.col.opt == "journal") {
                                        col_vec <- c(
                                            "#0072B2", "#E69F00", "#009E73", "#56B4E9", "#D55E00", "#CC79A7", "#E64B35FF", "#3C5488FF", "#F39B7FFF",
        ...
        pca_col_vec <- c(
            "mediumpurple4", "mediumpurple1", "blueviolet", "darkblue", "blue", "cornflowerblue", "cyan4", "skyblue",
            "darkgreen", "seagreen1", "green", "yellow", "orange", "pink", "coral1", "palevioletred2",
            "red", "saddlebrown", "brown", "brown3", "white", "darkgray", "aliceblue",
            "aquamarine", "aquamarine3", "bisque", "burlywood1", "lavender", "khaki3", "black"
        )
        ...
        feat_sds <- apply(data_m, 1, function(x) {
            sd(x, na.rm = TRUE)
        })
            ...
            if (logistic_reg == TRUE) {
                if (robust.estimate == FALSE) {
                    ...
                    if (robust.estimate == TRUE) {
                        output_dir <- paste(output_dir1, "logitregrobust", "signalthresh", group.missing.thresh, "RSD", log2.fold.change.thresh, "/", sep = "")
                    }
                }
            } else {
                if (poisson_reg == TRUE) {
                ...
                if (dim(data_m_fc)[1] > 0) {
                    ...
                    if (analysismode == "classification") {
                        ...
                        if (pca.global.eval == TRUE) {
                            ...
                            c1 = try(get_pcascoredistplots(
                                X = data_m_fc_withfeats, Y = classlabels_orig, feature_table_file = NA, parentoutput_dir = getwd(), class_labels_file = NA, sample.col.opt = sample.col.opt,
                        ...
                        if (pca.global.eval == TRUE) {
                            ...
                            c1 = try(get_pcascoredistplots(
                                X = data_m_fc_withfeats, Y = classlabels_orig, feature_table_file = NA, parentoutput_dir = getwd(), class_labels_file = NA,
                    ...
                    if (featselmethod == "limma" | featselmethod == "limma1way") {
                        ...
                        if (limma.contrasts.type == "contr.sum") {
                            ...
                            cnames_contr_factor1 <- apply(contrasts_factor1, 2, function(x) {
                                paste(names(x[which(abs(x) == 1)]), collapse = "-")
                            })
                        } else {
                            ...
                            cnames_contr_factor1 <- apply(contrasts_factor1, 2, function(x) {
                                paste(names(x[1]), names(x[which(abs(x) == 1)]), sep = "-")
                            })
                        }
                    ...
                    if (featselmethod == "limma2way") {
                        ...
                        if (limma.contrasts.type == "contr.sum") {
                            ...
                            cnames_contr_factor2 <- apply(contrasts_factor2, 2, function(x) {
                                paste(names(x[which(abs(x) == 1)]), collapse = "-")
                            })
                        } else {
                            contrasts_factor1 <- contr.treatment(length(levels(factor(Factor1))))
                    ...
                    if (featselmethod == "RF") {
                        ...
                        # xaxt="n",
                        x = barplot(sorted_varimp_res,
                            xlab = "", main = barplot_text, cex.axis = 0.9,
                            cex.names = 0.9, ylab = "", las = 2, ylim = range(pretty(c(0, sorted_varimp_res)))
                        )
                        title(
                            ylab = "VIM", cex.lab = 1.5,
                            line = 4.5
                        )
                    ...
                    if (featselmethod == "pls" | featselmethod == "o1pls" | featselmethod == "o2pls" | featselmethod == "spls" | featselmethod == "o1spls" | featselmethod == "o2spls") {
                        ...
                        if (featselmethod == "spls" | featselmethod == "o1spls" | featselmethod == "o2spls") {
                            ...
                            if (pairedanalysis == TRUE) {
                                ...
                                plsres1 <- do_plsda(
                                    X = data_m_fc, Y = classlabels_sub, oscmode = featselmethod, numcomp = pls_ncomp, kfold = kfold, evalmethod = pred.eval.method, keepX = max_varsel, sparseselect = TRUE,
                                ...
                                if (is.na(pls.permut.count) == FALSE) {
                                    ...
                                    if (pls.permut.count > 0) {
                                        ...
                                        rand_pls_sel <- parLapply(cl, 1:pls.permut.count, function(x) {
                                            ...
                                            plsresrand <- do_plsda_rand(
                                                X = data_m_fc, Y = classlabels_sub[sample(
                                                    x = seq(1, dim(classlabels_sub)[1]),
                                                    size = dim(classlabels_sub)[1]
                                                ), ], oscmode = featselmethod,
                                                ...
                                                optselect = FALSE, class_labels_levels_main = class_labels_levels_main, legendlocation = legendlocation, plotindiv = FALSE, alphabetical.order = alphabetical.order
                                            ) # ,silent=TRUE)
                                ...
                                set.seed(999)
                                plsres1 <- do_plsda(
                                    X = data_m_fc, Y = classlabels, oscmode = featselmethod, numcomp = pls_ncomp, kfold = kfold, evalmethod = pred.eval.method,
                                ...
                                if (is.na(pls.permut.count) == FALSE) {
                                    ...
                                    if (pls.permut.count > 0) {
                                        ...
                                        rand_pls_sel <- parLapply(cl, 1:pls.permut.count, function(x) {
                                            ...
                                            plsresrand <- do_plsda_rand(
                                                X = data_m_fc, Y = classlabels[sample(x = seq(1, dim(classlabels)[1]), size = dim(classlabels)[1]), ], oscmode = featselmethod, numcomp = opt_comp, kfold = kfold,
                            ...
                            if (pairedanalysis == TRUE) {
                                classlabels_temp <- cbind(classlabels_sub[, 2], classlabels)
                                plsres1 <- do_plsda(
                                    X = data_m_fc, Y = classlabels_temp, oscmode = featselmethod, numcomp = pls_ncomp, kfold = kfold, evalmethod = pred.eval.method,
                                ...
                                plsres1 <- do_plsda(
                                    X = data_m_fc, Y = classlabels, oscmode = featselmethod, numcomp = pls_ncomp, kfold = kfold, evalmethod = pred.eval.method, keepX = max_varsel,
                                ...
                                if (is.na(pls.permut.count) == FALSE) {
                                    ...
                                    if (pls.permut.count > 0) {
                                        ...
                                        rand_pls_sel <- parLapply(cl, 1:pls.permut.count, function(x) {
                                            ...
                                            plsresrand <- do_plsda_rand(
                                                X = data_m_fc, Y = classlabels[sample(x = seq(1, dim(classlabels)[1]), size = dim(classlabels)[1]), ],
                                                ...
                                                class_labels_levels_main = class_labels_levels_main, legendlocation = legendlocation, plotindiv = FALSE, alphabetical.order = alphabetical.order
                                            ) # ,silent=TRUE)
                    ...
                    if (featselmethod == "lmreg" | featselmethod == "lm1wayanova" | featselmethod == "lm2wayanova" | featselmethod == "lm1wayanovarepeat" | featselmethod == "lm2wayanovarepeat" | featselmethod == "logitreg" | featselmethod == "wilcox" | featselmethod == "ttest" |
                        ...
                        if (featselmethod == "lm2wayanova") {
                            ...
                            fdr_adjust_pvalue <- apply(fdr_adjust_pvalue, 1, function(x) {
                                min(x, na.rm = TRUE)
                            })
                            ...
                            fdr_adjust_pvalue_all <- apply(fdr_matrix, 1, function(x) {
                                return(min(x, na.rm = TRUE)[1])
                            })

                            pvalues <- cbind(pvalues1, pvalues2, pvalues3)
                            pvalues <- apply(pvalues, 1, function(x) {
                                min(x, na.rm = TRUE)[1]
                            })
                            ...
                            if (length(which(fdr_adjust_pvalue1 < fdrthresh)) > 0) {
                                ...
                                hca_f1 <- get_hca(
                                    feature_table_file = NA, parentoutput_dir = output_dir, class_labels_file = NA, X = X1, Y = Y1, heatmap.col.opt = heatmap.col.opt, cor.method = cor.method, is.data.znorm = FALSE,
                                    analysismode = "classification",
                            ...
                            if (length(which(fdr_adjust_pvalue2 < fdrthresh)) > 0) {
                                ...
                                hca_f2 <- get_hca(
                                    feature_table_file = NA, parentoutput_dir = output_dir, class_labels_file = NA, X = X2, Y = Y2, heatmap.col.opt = heatmap.col.opt, cor.method = cor.method, is.data.znorm = FALSE, analysismode = "classification",
                            ...
                            if (length(which(fdr_adjust_pvalue3 < fdrthresh)) > 0) {
                                ...
                                hca_f3 <- get_hca(
                                    feature_table_file = NA, parentoutput_dir = output_dir, class_labels_file = NA, X = X3, Y = Y3, heatmap.col.opt = heatmap.col.opt, cor.method = cor.method, is.data.znorm = FALSE, analysismode = "classification",
                                    sample.col.opt = sample.col.opt, plots.width = 2000, plots.height = 2000, plots.res = 300, alphacol = 0.3,
                        ...
                        if (featselmethod == "lm2wayanovarepeat") {
                            ...
                            pvalue_all <- apply(pvalue_matrix, 1, function(x) {
                                return(min(x, na.rm = TRUE)[1])
                            })
                            ...
                            if (length(which(fdr_adjust_pvalue1 < fdrthresh)) > 0) {
                                ...
                                hca_f1 <- get_hca(
                                    feature_table_file = NA, parentoutput_dir = output_dir, class_labels_file = NA, X = X1, Y = Y1, heatmap.col.opt = heatmap.col.opt, cor.method = cor.method, is.data.znorm = FALSE, analysismode = "classification",
                                    sample.col.opt = sample.col.opt, plots.width = 2000, plots.height = 2000, plots.res = 300,
                    ...
                    if (analysismode == "classification") {
                        ...
                        if (length(goodip) > 0 & dim(data_m_fc)[2] >= kfold) {
                            ...
                            dataA[, -c(1)] <- apply(dataA[, -c(1)], 2, function(x) {
                            ...
                            df.summary <- aggregate(x = dataA, by = list(as.factor(dataA$Targetvar)), function(x) {
                                mean(x, na.rm = TRUE)
                            })

                            # save(dataA,file="errordataA.Rda")
                            df.summary.sd <- aggregate(x = dataA[, -c(1)], by = list(as.factor(dataA$Targetvar)), function(x) {
                                sd(x, na.rm = TRUE)
                            })
                            ...
                            if (featselmethod == "lmreg" | featselmethod == "lm1wayanova" | featselmethod == "lm2wayanova" | featselmethod == "lm1wayanovarepeat" | featselmethod == "lm2wayanovarepeat" |
                                ...
                                try(get_manhattanplots(
                                    xvec = d4$mz, yvec = logp, ythresh = ythresh, up_or_down = zvec, xlab = "mass-to-charge (m/z)", ylab = ylabel,
                                    xincrement = x1increment, yincrement = yincrement, maintext = maintext1, col_seq = c("black"), y2thresh = y2thresh, colorvec = manhattanplot.col.opt
                                ), silent = TRUE)
                        ...
                        if (length(goodip) > 2) {
...

                            {
                                ...
                                if (output.device.type != "pdf") {
                                    ...
                                    hca_res <- get_hca(
                                        feature_table_file = NA, parentoutput_dir = output_dir, class_labels_file = NA, X = goodfeats, Y = classlabels_orig, heatmap.col.opt = heatmap.col.opt,
                                    ...
                                    # Generate HCA for selected features
                                    hca_res <- get_hca(
                                        feature_table_file = NA, parentoutput_dir = output_dir, class_labels_file = NA, X = goodfeats, Y = classlabels_orig, heatmap.col.opt = heatmap.col.opt, cor.method = cor.method, is.data.znorm = FALSE, analysismode = "classification",
                                        sample.col.opt = sample.col.opt, plots.width = 2000, plots.height = 2000, plots.res = 300, alphacol = 0.3, hca_type = hca_type, newdevice = FALSE,
                                        input.type = "intensity", mainlab = "", alphabetical.order = alphabetical.order, study.design = analysistype,
                                        labRow.value = labRow.value, labCol.value = labCol.value, similarity.matrix = similarity.matrix, cexLegend = hca.cex.legend, cexRow = cex.plots, cexCol = cex.plots
                                    )
                        ...
                        if (length(goodip) >= 1) {
                            ...
                            if (featselmethod == "lmreg" | featselmethod == "lm1wayanova" | featselmethod == "lm2wayanova" | featselmethod == "lm1wayanovarepeat" | featselmethod == "lm2wayanovarepeat" |
                                ...
                                try(get_manhattanplots(
                                    xvec = d4$mz, yvec = logp, ythresh = ythresh, up_or_down = zvec, xlab = "mass-to-charge (m/z)", ylab = "-logP", xincrement = x1increment, yincrement = 1,
                                    maintext = maintext1, col_seq = c("black"), y2thresh = (-1) * log10(pvalue.thresh), colorvec = manhattanplot.col.opt
                                ), silent = TRUE)
                                ...
                                try(get_manhattanplots(
                                    xvec = d4$time, yvec = logp, ythresh = ythresh, up_or_down = zvec, xlab = "Retention time", ylab = "-logP", xincrement = x2increment, yincrement = 1,
                                    maintext = maintext2, col_seq = c("black"), y2thresh = (-1) * log10(pvalue.thresh), colorvec = manhattanplot.col.opt
                                ), silent = TRUE)
                            ...
                            subdata <- apply(subdata, 2, function(x) {
                                naind <- which(is.na(x) == TRUE);if (length(naind) > 0) {
                                    x[naind] <- data_minval
                                };return(x)
                            })
                        ...
                        if (length(which(sel.diffdrthresh == TRUE)) > 2) {
                            ...
                            if (is(hr, "try-error") || is(hc, "try-error")) {
                                ...
                                if (znormtransform == FALSE) {
                                    h73 <- heatmap.2(data_m,
                                        Rowv = as.dendrogram(hr), Colv = as.dendrogram(hc), col = heatmap_cols, scale = "row", key = TRUE, symkey = FALSE,
                                    ...
                                    h73 <- heatmap.2(data_m,
                                        Rowv = as.dendrogram(hr), Colv = as.dendrogram(hc), col = heatmap_cols, scale = "none", key = TRUE,
                    ...
                    if (analysismode == "classification") {
                        ...
                        if (degree_rank_method == "diffrank") {
                            ...
                            degree_eval_res <- diffrank_eval(
                                X = data_m_fc_withfeats, Y = classlabels_temp, sigfeats = sigfeats, sigfeatsind = goodip,
                                ...
                                degree.centrality.method = degree.centrality.method, networktype = networktype
                            ) # ,silent=TRUE)
                        } else {
                            degree_eval_res <- {}
                        }
                            ...
                            if (pvalue.dist.plot == TRUE) {
                                ...
                                simpleQQPlot = function(observedPValues, mainlab) {
                                    plot(-log10(1:length(observedPValues) / length(observedPValues)),
                                        ...
                                        main = mainlab, xlab = paste("Expected -log10pvalue", sep = ""), ylab = paste("Observed -logpvalue", sep = ""), cex.main = 0.75
                                    )
                        ...
                        if (analysismode == "regression") {
                            ...
                            if (num_sig_feats >= 3 & pca.stage2.eval == TRUE) {
                                ...
                                get_pcascoredistplots(
                                    X = goodfeats, Y = classlabels_orig, feature_table_file = NA, parentoutput_dir = getwd(),
                                    ...
                                    alphabetical.order = alphabetical.order, study.design = analysistype, lme.modeltype = modeltype
                                ) # ,silent=TRUE)
                    ...
                    if (analysismode == "classification" && nrow(goodfeats) >= 1 && length(goodip) >= 1) {
                        if (is.na(rocclassifier) == FALSE) {
                            if (length(class_labels_levels) == 2) {
                                ...
                                roc_res <- try(get_roc(
                                    dataA = goodfeats_temp, classlabels = classlabels, classifier = rocclassifier, kname = "radial",
                        ...
                        if (num_sig_feats >= 3 & pca.stage2.eval == TRUE) {
                            ...
                            get_pcascoredistplots(
                                X = goodfeats_temp, Y = classlabels_orig_pca,
                                ...
                                lineplot.col.opt = lineplot.col.opt, lineplot.lty.option = lineplot.lty.option, timeseries.lineplots = timeseries.lineplots, pcacenter = pcacenter, pcascale = pcascale, alphabetical.order = alphabetical.order, study.design = analysistype, lme.modeltype = modeltype
                            ) # ,silent=TRUE)
                        ...
                        # if(FALSE)
                        {
                            ...
                            if (pairedanalysis == TRUE || timeseries.lineplots == TRUE) {
                                ...
                                # try(
                                var_sum_list <- get_lineplots(
                                    X = goodfeats_temp, Y = classlabels_orig, feature_table_file = NA,
                        ...
                        # else
                        {
                            ...
                            if (generate.boxplots == TRUE) {
                                ...
                                if (normalization.method != "none") {
                                    ...
                                    if (pairedanalysis == TRUE) {
                                        ...
                                        res <- get_boxplots(
                                            X = goodfeats_temp, Y = classlabels_orig, parentoutput_dir = output_dir, boxplot.col.opt = boxplot.col.opt,
                                ...
                                if (plot.boxplots.raw == TRUE) {
                                    ...
                                    if (pairedanalysis == TRUE) {
                                        ...
                                        get_boxplots(
                                            X = goodfeats_raw, Y = classlabels_orig, parentoutput_dir = output_dir, boxplot.col.opt = boxplot.col.opt,
                            ...
                            if (FALSE) {
                                ...
                                try(get_barplots(feature_table_file, class_labels_file,
                                    X = goodfeats_temp, Y = classlabels_orig, parentoutput_dir = output_dir,
                                    newdevice = FALSE, ylabel = ylab_text, cex.val = cex.plots, barplot.col.opt = barplot.col.opt, error.bar = error.bar
                                ), silent = TRUE)
                                ...
                                get_barplots(feature_table_file, class_labels_file,
                                    X = goodfeats_temp, Y = classlabels_orig, parentoutput_dir = output_dir,
                            ...
                            if (globalclustering == TRUE) {
                                ...
                                # if(FALSE)
                                {
                                    ...
                                    h73 <- heatmap.2(as.matrix(data_m_fc_withfeats[, -c(1:2)]),
                                        Rowv = as.dendrogram(hr), Colv = as.dendrogram(clust1),
        ...
        if (length(best_feats) > 1) {
            ...
            if (analysismode == "regression") {
                ...
                get_scatter_plots(
                    X = goodfeats_temp, Y = classlabels_orig, parentoutput_dir = output_dir, newdevice = FALSE, ylabel = ylab_text, xlabel = xlab_text,
                    ...
                    if (is.na(abs.cor.thresh) == FALSE) {
                        ...
                        if (globalcor == TRUE) {
                            ...
                            if (networktype == "complete") {
                                ...
                                mwan_fdr <- try(do_cor(data_matrix,
                                    subindex = sigfeats_index, targetindex = NA, outloc = output_dir, networkscope = "global", cor.method, abs.cor.thresh, cor.fdrthresh,
                                    max.cor.num, net_node_colors, net_legend, newdevice = TRUE
                                ), silent = TRUE)
                            } else {
                                if (networktype == "GGM") {
                                    mwan_fdr <- try(get_partial_cornet(data_matrix,
                                        sigfeats.index = sigfeats_index, targeted.index = NA, networkscope = "global",
                                        cor.method, abs.cor.thresh, cor.fdrthresh, outloc = output_dir, net_node_colors, net_legend, newdevice = TRUE
                                    ), silent = TRUE)
                                } else {
        ...
        return(list(
            "diffexp_metabs" = goodfeats_allfields, "mw.an.fdr" = mwan_fdr, "targeted.an.fdr" = targetedan_fdr,
