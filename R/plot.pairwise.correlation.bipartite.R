plot.pairwise.correlation.bipartite <-
    function(data_matrix, data_matrixB, newdevice = TRUE, abs.cor.thresh = 0.4, pvalue.thresh = 0.05,
        ...
        corpval1 = apply(cor1, 2, function(x) {
            corPvalueStudent(x, n = ncol(goodfeats_temp[, -c(col_ind_rm)]))
        })
        ...
        if (do.clustering == TRUE) {
            h1 <- heatmap.2(cor1,
                col = rev(brewer.pal(11, "RdBu")), Rowv = TRUE, Colv = TRUE, scale = "none", key = TRUE, symkey = FALSE,
                density.info = "none", trace = "none", main = mainlab1, cexRow = 0.5, cexCol = 0.5, cex.main = 0.7,
                xlab = "Dataset B", ylab = "Dataset A"
            )
        } else {
            h1 <- heatmap.2(cor1,
                col = rev(brewer.pal(11, "RdBu")), Rowv = FALSE, Colv = FALSE, scale = "none", key = TRUE, symkey = FALSE,
                density.info = "none", trace = "none", main = mainlab1, cexRow = 0.5, cexCol = 0.5, cex.main = 0.7,
