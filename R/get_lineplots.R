            ...
            if (dim(classlabels_orig)[2] > 2) {
                ...
                if (FALSE) {
                    ...
                    myData_sum_cor <- lapply(levels_groups, function(g) {
                        ...
                        y = as.data.frame(c(
                            group = g, mean.Pearson.cor = mean_val,
                ...
                plot_res <- lapply(seq(3, dim(myData_sum)[2], 4), function(pc) {
                    ...
                    if (pairedanalysis == TRUE) {
                        ...
                        if (timeseries.lineplots == TRUE) {
                            ...
                            # if(FALSE)
                            {
                                ...
                                if (is.na(lineplot.lty.option) == TRUE) {
                                    plot_res <- suppressWarnings(ggplot(df.summary, aes(
                                        x = as.factor(x), y = Intensity, colour = Class,
                                        linetype = Class
                                    )) +
                            ...
                            plot_res <- ggplot(df.summary, aes(x = as.factor(x), y = Intensity, color = Class)) +
                                geom_point(size = pca.cex.val, shape = shape_vec1) +
                                geom_errorbar(aes(ymin = ymin, ymax = ymax), size = sizeval) +
                ...
                # for(pc in seq(2,dim(myData_sum)[2],4))
                {
                    ...
                    if (pairedanalysis == FALSE) {
                        if (timeseries.lineplots == FALSE) {
                            ...
                            options(repr.plot.width = 3, repr.plot.height = 2)
                            plot_res <- ggplot(df.summary, aes(x = Class, y = Intensity, color = Class)) +
                                geom_point(size = pca.cex.val) +
                                geom_errorbar(aes(ymin = ymin, ymax = ymax), size = 0.3, width = 0.1) +
                                ylab(ylabel) +
                                scale_x_continuous(breaks = seq(1, length(class_levels))) +
                                scale_color_manual(values = col_vec) +
                                theme_bw() +
                                theme(
                                    panel.border = element_blank(), panel.grid.major = element_blank(),
                            ...
                            # ylim(0,max(ymax,na.rm=TRUE)) +
                            options(repr.plot.width = 3, repr.plot.height = 2)
                            plot_res <- ggplot(df.summary, aes(x = Class, y = Intensity, group = 1)) +
                                geom_point(size = pca.cex.val) +
                        ...
                        plot_res <- ggplot(df.summary, aes(x = Class, y = Intensity, group = 1)) +
                            geom_point(size = pca.cex.val) +
                            geom_line(size = sizeval) +
                            geom_errorbar(aes(ymin = ymin, ymax = ymax), width = 0.2, size = sizeval) +
                            ylab(ylabel) +
                            scale_color_manual(values = col_vec) +
                            theme_bw() +
                            theme(
                ...
                if (multiple.figures.perpanel == TRUE) {
                    res <- lapply(seq(1, length(plot_res), 4), function(i) {
                        ...
                        figure <- ggarrange(p1, p2, p3, p4,
                            ncol = 2, nrow = 2, heights = c(4, 4), width = c(6, 6),
                    ...
                    res <- lapply(seq(1, length(plot_res), 1), function(i) {
                        ...
                        figure <- ggarrange(p1,
                            ncol = 1, nrow = 1, heights = c(4, 4), width = c(6, 6),
                            legend = FALSE, align = c("hv")
                        )
                        return(figure)
                    })
