metabnet <-
    function(feature_table_file, target.metab.file, sig.metab.file, class_labels_file = NA, parentoutput_dir, num_replicates = 1, cor.method = "spearman", abs.cor.thresh = 0.4, cor.fdrthresh = 0.05,
        ...
        data_matrix <- data_preprocess(
            Xmat = NA, Ymat = NA, feature_table_file = feature_table_file, parentoutput_dir = parentoutput_dir, class_labels_file = NA, num_replicates = num_replicates, feat.filt.thresh = NA,
        ...
        if (is.na(target.metab.file) == FALSE) {
            ...
            g1 <- getVenn(
                dataA = dataA, name_a = "TargetSet", name_b = "ExperimentalSet", dataB = data_matrix[, c(1:2)], mz.thresh = target.mzmatch.diff, time.thresh = target.rtmatch.diff,
            ...
            if (length(g1$common) > 1) {
                ...
                if (networktype == "complete") {
                    targetedan_fdr <- do_cor(data_matrix,
                        subindex = sigfeats.index, targetindex = g1$common$index.B, outloc, networkscope = "targeted", cor.method,
                    ...
                    if (networktype == "GGM") {
                        targetedan_fdr <- get_partial_cornet(data_matrix, sigfeats.index,
                            targeted.index = g1$common$index.B, networkscope = "targeted", cor.method, abs.cor.thresh,
            ...
            if (networktype == "complete") {
                ...
                targetedan_fdr <- do_cor(data_matrix,
                    subindex = sigfeats.index, targetindex = NA, outloc, networkscope = "global", cor.method, abs.cor.thresh, cor.fdrthresh,
                ...
                if (networktype == "GGM") {
                    targetedan_fdr <- get_partial_cornet(data_matrix, sigfeats.index,
                        targeted.index = NA, networkscope = "global", cor.method, abs.cor.thresh,
                        cor.fdrthresh, outloc = outloc, net_node_colors, net_legend, netrandseed
                    )
