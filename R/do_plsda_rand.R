        ...
        if (sparseselect == TRUE) {
                ...
                if (optselect == TRUE) {
                    ...
                    for (kvec in keepx_seq) {
                        ...
                        if (repeatmeasures == TRUE) {
                            ...
                            linn.pls <- try(mixOmics::multilevel(
            ...
            if (analysismode == "regression") {
                if (repeatmeasures == TRUE) {
                    ...
                    linn.pls <- try(mixOmics::multilevel(
                        X = X, Y = Y, design = classlabels[, 2], ncomp = opt_comp,
                        keepX = keep_x_vec, method = 'spls'
                    ), silent = TRUE)
                ...
                if (repeatmeasures == TRUE) {
                    ...
                    if (FALSE) {
                        stimu.time <- data.frame(cbind(
                            as.character(classlabels[, 2]),
                            as.character(classlabels[, 3])
                        ))
                        repeat.stimu2 <- classlabels[, 1]

                        res.2level <- multilevel(X,
                            cond = stimu.time,
