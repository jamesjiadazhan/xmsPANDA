get_scatter_plots <-
    function(X = NA, Y = NA, feature_table_file, parentoutput_dir,
        ...
        if (is.na(X[1]) == TRUE) {
            data_matrix <- read.table(feature_table_file,
                sep = "\t",
                header = TRUE
            )
        } else {
        ...
        if (is.na(group_by_mat_file[1]) == FALSE) {
            group_by_mat <- read.table(group_by_mat_file,
                sep = "\t",
                header = TRUE
            )
        }
                        ...
                        if (sample.col.opt == "terrain") {
                            ...
                            if (sample.col.opt == "colorblind") {
                                ...
                                if (length(check_brewer) > 0) {
                                    sample.col.opt_temp = gsub(
                                        x = sample.col.opt,
                                        pattern = "brewer.", replacement = ""
                                    )
                                    col_vec <- colorRampPalette(brewer.pal(
                                        10,
                                        sample.col.opt_temp
                                    ))(length(class_labels_levels))
                                } else {
        ...
        if (length(check_names) > 0) {
            if (check_names == 1) {
                ...
                if (length(check_names1) < 1 & length(check_names2) <
                    ...
                    write.table(names_with_mz_time,
                        file = "Stage1/Name_mz_time_mapping.txt",
                        sep = "\t", row.names = FALSE
                    )
                    X <- as.data.frame(X)
                } else {
        ...
        lapply(1:dim(goodfeats)[1], function(m) {
            ...
            if (is.na(group_by_mat[1]) == TRUE) {
                p <- suppressMessages(ggscatter(temp_dm,
                    y = "Class", x = "Feature", conf.int = conf.int,
                ...
                if (multiple.facets == TRUE) {
                    p <- suppressMessages(ggscatter(temp_dm,
                        ...
                        hjust = 0.5,
                        size = 10
                    )) + stat_cor(method = cor.method, aes(color = GroupBy))
                } else {
                    # ylim=c(s1[1],s1[6]),
