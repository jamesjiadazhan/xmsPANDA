diffexp.biomarkers1 <-
    function(X = NA, Y = NA, feature_table_file = NA, class_labels_file = NA,
        ...
        if (is.na(good_feats_index) == TRUE) {
            if (is.na(feat.sel.methods) == FALSE) {
                ...
                for (m in 1:length(feat.sel.methods)) {
                    ...
                    stability_measure <- apply(v1, 1, function(x) {
                        length(which(x == 1)) / length(x)
                    }) # quantile(x,iter.quantile.thresh)})
        ...
        if (length(good_feats_index) > 1) {
            ...
            barplot(eval_mat_1,
                beside = TRUE, main = "Comparison of CV accuracy using different classifiers\n based on learning sets",
            ...
            if (length(good_feats_index) >= 3) {
                ...
                try(get_hca(
                    parentoutput_dir = outloc, X = good_feats, Y = Y1, heatmap.col.opt = heatmap.col.opt, cor.method = "spearman", is.data.znorm = FALSE, analysismode = "classification",
                    sample.col.opt = "rainbow", plots.width = 2000, plots.height = 2000, plots.res = 300, alphacol = 0.3, hca_type = hca_type,
                    newdevice = FALSE, labRow.value = labRow.value, labCol.value = labCol.value, similarity.matrix = similarity.matrix, cexLegend = hca.cex.legend, cexRow = cex.plots, cexCol = cex.plots
                ), silent = TRUE)
            ...
            get_boxplots(
                X = good_feats, Y = Y1, parentoutput_dir = outloc, boxplot.col.opt = boxplot.col.opt,
            ...
            # if(is.na(Xtest)==FALSE)
            {
                ...
                eval_mat_perm <- lapply(1:nperm, function(p1) {
                    ...
                    temp_res <- c(
                        100 * (1 - mean(testeval_res1@score)), 100 * (1 - mean(testeval_res2@score)), permkfold_acc3,
                ...
                if (is.na(Xtest) == FALSE) {
                    ...
                    if (length(good_feats_index) > 2) {
                        ...
                        if (deeplearning == TRUE) {
                            ...
                            amd.dl <- h2o.deeplearning(
                                x = 1:(ncol(train_hex_split[[1]]) - 1), y = "Class", seed = 1234567, training_frame = as.h2o(train_hex_split[[1]]), activation = best_model@parameters$activation, epoch = best_model@parameters$epochs, l1 = best_model@parameters$l1, l2 = best_model@parameters$l2, rho = best_model@parameters$rho, epsilon = best_model@parameters$epsilon,
            ...
            if (globalcor == TRUE) {
                ...
                if (networktype == "complete") {
                    ...
                    if (networktype == "GGM") {
                        mwan_fdr <- get_partial_cornet(data_m_fc_withfeats,
                            sigfeats.index = sigfeats_index, targeted.index = NA, networkscope = "global", cor.method,
