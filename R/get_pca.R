    ...
    function(X, samplelabels, legendlocation = "topright", filename = NA, ncomp = 5, pcacenter = TRUE, pcascale = TRUE, legendcex = 0.5,
        ...
        if (pcascale == "pareto") {
            X <- apply(X, 2, function(x) {
                y <- (x - mean(x, na.rm = TRUE)) / sqrt(sd(x, na.rm = TRUE));return(y)
            })
        ...
        # if(pca.ellipse=="car")
        {
            ...
            plotIndiv(result,
                comp = c(1, 2), group = as.factor(samplelabels), legend = TRUE,
            ...
            plotIndiv(result,
                comp = c(1, 2), group = as.factor(samplelabels), legend = TRUE, ellipse = pca.ellipse,
            ...
            if (ncomp_plot > 2) {
                ...
                plotIndiv(result,
                    comp = c(1, 3), group = as.factor(samplelabels), legend = TRUE, ellipse = pca.ellipse, style = "lattice",
                    title = paste("PCA using ", filename, "features (with names)", sep = ""), col.per.group = col_per_group
                )
                # ,ind.names=FALSE)
        ...
        if (nrow(result$loadings$X) < 30) {
            ...
            cex.plots = 0.8
            p = ggplot(df.long.prcomp, aes_string(x = "varName", y = "loading", ymax = "loading")) +
                geom_point() +
                geom_linerange(aes(ymin = 0)) +
                facet_wrap(~PC, nrow = 1) +
                coord_flip() +
                ggtitle("variable loadings for PCs")
            ...
            p = ggplot(df.long.prcomp, aes_string(x = "varName", y = "loading", ymax = "loading")) +
                ...
                facet_wrap(~PC, nrow = 1) +
                coord_flip() +
                ggtitle("top 30 variable loadings for PCs")
            p = p + theme_bw() + theme(
                panel.border = element_blank(), panel.grid.major = element_blank(),
                panel.grid.minor = element_blank(),
                panel.spacing = unit(1, "lines"),
