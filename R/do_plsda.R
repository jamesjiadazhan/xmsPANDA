    ...
    function(X, Y, oscmode = "pls", numcomp = 3, kfold = 10, evalmethod = "CV", keepX = 15, sparseselect = FALSE, analysismode = "classification",
        ...
        if (sparseselect == TRUE) {
                ...
                if (optselect == TRUE) {
                    ...
                    for (kvec in keepx_seq) {
                        ...
                        if (repeatmeasures == TRUE) {
                            linn.pls <- try(multilevel(
                                X = X, design = classlabels, ncomp = opt_comp,
            ...
            if (analysismode == "regression") {
                ...
                if (repeatmeasures == TRUE) {
                    ...
                    linn.pls <- try(multilevel(
                        X = X, design = classlabels[, 2], ncomp = opt_comp,
        ...
        if (analysismode == "classification") {
            ...
            if (plotindiv == TRUE) {
                ...
                # print(Sys.time())
                get_plsplots(X,
                    plsres = linn.pls, plsvar = pls_var, samplelabels = Yclass, filename = NA, ncomp = opt_comp, center = TRUE, scale = TRUE, legendcex = 0.5, outloc = getwd(), col_vec = col.stimu,
