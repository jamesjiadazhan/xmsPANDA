    ...
    function(adjMtrxSample1, adjMtrxSample2, degree.centrality.method = "eigenvector", node_names = NA, class_labels = c(1, 2),
        ...
        if (plot_graph_bool == TRUE) {
            ...
            if (plot.layout.type == "circular") {
                ...
                set.seed(555)
                plot.igraph(graph1,
                    main = main_graph1, vertex.label = V(graph1)$name, vertex.label.cex = label.cex, edge.color = edge_colors,
                ...
                set.seed(555)
                plot.igraph(graph2,
                    main = main_graph2, vertex.label = V(graph2)$name, vertex.label.cex = label.cex, edge.color = edge_colors,
                    vertex.size = vertex.size, layout = coords2, vertex.color = membership(c2)
                )
            } else {
                state_col = c(
                    "TssA" = "#E41A1C", "TssAFlnk" = "#E41A1C",
