...
data_preprocess <- function(
    ...
    if (summarize.replicates == TRUE) {
        if (num_replicates > 1) {
            ...
            filename <- switch(summary.method,
                "mean" = {
                    print("Replicate averaging done"); "Rawdata_averaged.txt"
                },
                "median" = {
                    print("Replicate median summarization done"); "Rawdata_median_summarized.txt"
                },
                NA
            )
