get_boxplots_child <- function(X, Y, feature_table_file, parentoutput_dir, class_labels_file, boxplot.col.opt = "journal", alphacol = 0.3, newdevice = TRUE, cex.plots = 0.8, replace.by.NA = FALSE, pairedanalysis = FALSE, filename = "", ylabel = "Intensity",
                               ...
                               boxplot.type = "ggplot",
                               study.design = c(
                                   "multiclass", "onewayanova", "twowayanova", "onewayanovarepeat",
                                   "twowayanovarepeat"
                               ),
    ...
    if (boxplot.type == "simple") {
        ...
        plot_res <- parLapply(
            cl, 1:dim(goodfeats)[1], function(m, mzvec, timevec, check_names, name, class_labels_levels, sampleclass, col_vec, goodfeats, pairedanalysis, connectpairedsamples, boxplot.type,
                            ...
                            if (TRUE) {
                                ...
                                if (multiple.groups == FALSE) {
                                    ...
                                    if (is.na(group_by_mat) == FALSE) {
                                        ...
                                        p <- ggplot(temp_dm2, aes(y = as.numeric(Feature), x = Class, fill = Class)) +
                                            labs(title = mzname) +
                                            facet_wrap(~GroupBy, scale = "free_x", nrow = facet.nrow, ncol = facet.ncol)
                                        ...
                                        if (is.na(ggplot.type1) == FALSE) {
                                            if (ggplot.type1 == TRUE) {
                                                ...
                                                p <- ggplot(temp_dm2, aes(y = as.numeric(Feature), x = Factor1, fill = Factor1)) +
                                                    labs(title = mzname) +
                                                    facet_wrap(~Factor2, scale = "free_x", nrow = facet.nrow, ncol = facet.ncol)
                                            } else {
                                                p <- ggplot(temp_dm2, aes(y = as.numeric(Feature), x = Factor2, fill = Factor2)) +
                                                    labs(title = mzname) +
                                                    facet_wrap(~Factor1, scale = "free_x", nrow = facet.nrow, ncol = facet.ncol)
                                            }
                                        } else {
                                            p <- ggplot(temp_dm2, aes(y = as.numeric(Feature), x = Factor1, fill = Factor2)) +
                                                labs(title = mzname)
                                        }
                                    } else {
                                        if (is.na(ggplot.type1) == FALSE) {
                                            if (ggplot.type1 == TRUE) {
                                                ...
                                                p <- ggplot(temp_dm2, aes(y = as.numeric(Feature), x = Factor1, fill = GroupBy)) +
                                                    labs(title = mzname) +
                                                    facet_wrap(~Factor2, scale = "free_x", nrow = facet.nrow, ncol = facet.ncol)
                                            } else {
                                                p <- ggplot(temp_dm2, aes(y = as.numeric(Feature), x = Factor2, fill = GroupBy)) +
                                                    labs(title = mzname) +
                                                    facet_wrap(~Factor1, scale = "free_x", nrow = facet.nrow, ncol = facet.ncol)
                                            }
                                        } else {
                                ...
                                if (add.pvalues == TRUE) {
                                    ...
                                    if (multiple.groups == FALSE) {
                                        p <- p + stat_compare_means(
                                            data = temp_dm2, aes(group = Class), size = 5 * cex.plots, label = "p.format",
                                        ...
                                        if (pairedanalysis == FALSE) {
                                            ...
                                            if (ggplot.type1 == TRUE) {
                                                if (is.na(ref.group.val) == TRUE) {
                                                    ...
                                                    if (ref.group.val == FALSE) {
                                                        p <- p + stat_compare_means(
                                                            data = temp_dm2, aes(group = Factor1), label = "p.format",
                                                            size = 5 * cex.plots
                                                        )
                                                    } else {
                                                        p <- p + stat_compare_means(
                                                ...
                                                p <- p + stat_compare_means(
                                                    data = temp_dm2, aes(group = Factor2), label = "p.format",
                                                    size = 5 * cex.plots, ref.group = ref.group.val
                                                )
                                            }
                                        } else {
                                            if (ggplot.type1 == TRUE) {
                                                ...
                                                p <- p + stat_compare_means(
                                                    data = temp_dm2, aes(group = Factor2), paired = TRUE, label = "p.format",
                                                    size = 5 * cex.plots
                                                )
                                            }
                                        }
                                ...
                                p <- p + labs(y = ylabel) + theme_bw() + theme(
                                ...
                                ) + scale_fill_manual(values = (col_vec[1:length(class_labels_levels)])) + scale_y_continuous(breaks = scales::pretty_breaks(n = 10)) + theme(axis.text.x = element_text(angle = 45, hjust = 1))

                                p = p + theme(
                                    legend.title = element_text(size = 13 * cex.plots),
        ...
        if (boxplot.type == "ggplot") {
            ...
            ggpubr::ggexport(res,
                filename = boxplots_fname, width = unit(plot.width - 0.5, "in"),
