plotTukeyHSD1 <-
    function(tukey.res,
        ...
        res1 <- lapply(1:length(tukey.res), function(j) {
            ...
            tmp1[, -c(1)] <- apply(tmp1[, -c(1)], 2, function(x) {
                as.numeric(as.character(x))
            })
            cex.plots = plot.cex.axis
            tmp1$categories <- factor(tmp1$categories, levels = unique(tmp1$categories))

            p1 = ggplot(tmp1, aes(y = means, x = categories, colour = factor(padjbool), show.legend = FALSE)) +
                geom_point() +
                ...
                theme_bw() +
                ylab(as.character(y.axis.label[j])) +
                xlab("Comparison")
            ...
            p1 = p1 + labs(fill = "p<0.05") + font("legend.text", size = 10 * cex.plots, color = "black") + theme(
            ...
            ) + scale_color_manual(values = c("blue", "red"), labels = c("padj>=0.05", "padj<0.05"))
        ...
        res2 <- lapply(1:1, function(j) {
            ...
            p1 = p1 + labs(fill = "p<0.05") + font("legend.text", size = 8 * cex.plots, color = "black") + theme(
                panel.border = element_blank(), panel.grid.major = element_blank(),
                panel.grid.minor = element_blank(),
                panel.spacing = unit(1, "lines"),
                axis.line = element_line(colour = "black", size = 1),
