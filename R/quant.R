quant <-
    function(Xmat = NA, Ymat = NA, Wmat = NA, Zmat = NA, feature_table, class_file, ref_list, foldchange_list, outloc,
        ...
        if (summarize_replicates == TRUE) {
            capture.output(avg_feature_table <- data_preprocess(
                X = feature.table, feature_table_file = NA, parentoutput_dir = outloc,
                ...
                normalization.method = normalization.method, alphabetical.order = alphabetical.order
            ), file = '/dev/null')
